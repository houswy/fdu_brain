...
h5create(output_name,dataset_name,[xdim,ydim,num_frame],"Chunksize",[xdim,ydim,1000],"Datatype","uint16");
...
%%
% Extract
M = h5read('0_mc_ds.h5','/images');
%M = h5read('0.h5','/images');
config = [];
config.use_gpu=0;
...
config.spatial_highpass_cutoff =3;
config.max_iter=10;
...
output_sort = output;
[traces,filters]=final_robust_run(M,output_sort,[output_sort.user_labels]==1);
output_sort_final=output_sort;
output_sort_final.spatial_weights=filters;
output_sort_final.temporal_weights=traces;
%%
%plot figure, show the quality
figure, imshow(max(M,[],3),[0 350]);
plot_cells_overlay(output_sort_final.spatial_weights,[1,0,0],[]);
plot_cells_overlay(output_sort_final.spatial_weights(:,:,18),[1,0,0],[]);