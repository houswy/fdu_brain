function object_cell_idx = objectcell_WF_psk(behavior, imaging, object)
...
for i = 1:length(trial_num)
    ...
    bins = behavior_use(i).WF_sto-behavior_use(i).WF_sta+1;
    H = [];
    H = zeros(bins,1)';
    trial_sta = behavior_use(i).WF_frames(idx(1)) - behavior_use(i).WF_sta + 1;
    trial_sto = behavior_use(i).WF_frames(idx(end)) - behavior_use(i).WF_sta + 1;
...
for j = 1:size(cellActivity,1)
    ...
    for i = 1:length(trial_num)
        ...
        G = row_data(behavior_use(i).WF_sta:behavior_use(i).WF_sto)
        J = [];
        J = rescale(G);
        J(J>=0.8) = 1;
        J(J<0.8) = 0;
...
%object_q_cell_idx = find(test<0.000000000000001);%算E53MINI C386WF
object_cell_idx = find(test<0.05);%算C597WF